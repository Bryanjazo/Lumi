...
export interface Quest {
  ...
  /**
   * ONE small physical first action ("clear just the desk surface")
   * so a paralyzed user sees the tiny step, not the mountain — v1 of
   * the onboarding promise "one small first step, never the whole
   * mountain". Generated by llmFirstStep or cleared by the user.
   *
   * DEVICE-LOCAL: there's no cloud column for it, so it never round-
   * trips to other devices. The pull merge DOES carry the prior local
   * value forward (see lib/sync.ts) so it survives a cold-start pull on
   * THIS device — without that it was wiped on every launch.
   */
...
interface QuestState {
  ...
  /**
   * Set (or clear with null / blank) the one small first step for a
   * quest. Device-local field — see Quest.firstStep; no calendar
   * mirror, no sync.
   */
  setFirstStep: (id: string, step: string | null) => void;
  ...
  /**
   * ISO time of the last SUCCESSFUL full quest push. The pull merge
   * reads it to distinguish an un-pushed local completion (keep it —
   * the cloud just hasn't caught up) from a genuine cross-device
   * un-complete that already reached the cloud (adopt it). Without it
   * an offline completion whose debounced push failed silently
   * reverted on the next pull. Null until the first clean push.
   */
  lastPushedAt: string | null;
  /** Stamp lastPushedAt — sync.ts calls this after a clean quest
   *  upsert, passing the snapshot time captured BEFORE the write so a
   *  completion made DURING the write still counts as un-pushed. */
  markQuestsPushed: (at: string) => void;
...
export const useQuestStore = create<QuestState>()(
  persist(
    (set, get) => ({
      ...
      setFirstStep: (id, step) => {
        // Same drop-the-field-when-empty pattern as setNote/setComment,
        // so a cleared step doesn't linger as an empty string. No
        // calendar mirror — the first step is a private nudge, not
        // event content.
      ...
      markQuestsPushed: (at) => set({ lastPushedAt: at }),
      ...
      refreshRecurring: () => {
        ...
        set((s) => ({
          quests: s.quests.map((q) => {
            if (!q.recur) return q;
            // Already dated today — this IS today's instance; never
            // reset it (completed or not). Stamp lastSpawnedDate while
            // we're here: a quest CREATED for a future due date (e.g.
            // "gym every Monday" added on a Wednesday) reaches its day
            // with lastSpawnedDate still at the creation date, so the
            // idempotency guard below missed it — completing the habit
            // and refocusing a tab un-completed it and re-farmed XP.
            if (q.date === today) {
              return q.lastSpawnedDate === today
                ? q
                : { ...q, lastSpawnedDate: today };
            }
